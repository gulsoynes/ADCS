...
for i = 1:p.N+1
    ...
    [X(:,i+1), C] = dynamics(X(:,i), p.sat.I, p.sat.Moment, p.dt);
    % Sensor measurement
    [b_o(:,i), b_b(:,i), s_o(:,i), s_b(:,i), n_o(:,i), n_b(:,i)]  = sensor_model(pos(i,:)', p.orb, p.sensor, C, t);
end
...
params.N = params.orb.T * 1 * 10;
params.tspan = 0:params.dt:params.N;
